...
const columns = (intl) => [
  ...
  },
  {
    ...
    transforms: [sortable, wrappable, cellWidth(25)],
    cellTransforms: [breakWord]
  },
  {
    title: intl.formatMessage(requestsMessages.requesterColumn),
    transforms: [sortable, wrappable, cellWidth(25)]
  },
  {
    title: intl.formatMessage(requestsMessages.updatedColumn),
    transforms: [cellWidth(15)]
  },
  {
    title: intl.formatMessage(requestsMessages.statusColumn),
...
const RequestsList = ({ persona, indexpath, actionResolver }) => {
  ...
  const {
    requests: { data, meta, count },
    sortBy,
    filterValue
  } = useSelector(
    ({ requestReducer: { requests, sortBy, filterValue } }) => ({
      requests,
      sortBy,
      filterValue
    }),
  ...
  const [
    { nameValue, isFetching, isFiltering, requesterValue, rows },
    stateDispatch
  ] = useReducer(
    requestsListState,
  ...
  const noRequestsMessage = () =>
    indexpath === routesLinks.allrequest
      ? intl.formatMessage(requestsMessages.emptyAllRequestsDescription)
  ...
  const routes = () => (
    <Fragment>
      <Route
        exact
        path={routesLinks.requests.comment}
        render={(props) => (
          <ActionModal
            {...props}
            actionType={'Comment'}
      ...
      <Route
        exact
        path={routesLinks.requests.approve}
        render={(props) => (
          <ActionModal
            {...props}
            actionType={'Approve'}
      ...
      <Route
        exact
        path={routesLinks.requests.deny}
        render={(props) => (
          <ActionModal
            {...props}
            actionType={'Deny'}
  ...
  useEffect(() => {
    stateDispatch({
      type: 'setRows',
      payload: createRows(actionResolver, data, indexpath, intl)
    });
  }, [data]);
  ...
  const onDeleteChip = ([
    ...
    const newValue = ['name', 'requester'].includes(key)
      ? ''
      : filterValue[key].filter((val) => value !== val);
  ...
  return (
    <Fragment>
      ...
      <TableToolbarView
        ...
        activeFiltersConfig={{
          filters: prepareChips(
            {
              name: nameValue,
              requester: requesterValue,
              decision: filterValue.decision
            },
            intl
          ),
          onDelete: (_e, chip, deleteAll) =>
            deleteAll ? clearFilters() : onDeleteChip(chip)
        }}
        filterConfig={[
          {
            ...
            filterValues: {
              placeholder: intl.formatMessage(
                ...
                {
                  title: intl
                    .formatMessage(requestsMessages.requesterColumn)
                    .toLowerCase()
                }
              ),
              ...
              onChange: (_event, value) =>
                handleFilterChange(value, 'requester'),
          ...
          },
          {
            ...
            filterValues: {
              placeholder: intl.formatMessage(
                ...
                {
                  title: intl
                    .formatMessage(requestsMessages.statusColumn)
                    .toLowerCase()
                }
              ),
              'aria-label': intl.formatMessage(
              ...
              onChange: (_event, value) =>
                handleFilterChange(value, 'decision'),
