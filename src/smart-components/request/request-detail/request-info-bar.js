    ...
    <Stack hasGutter>
          ...
          <CardBody>
            <Stack hasGutter>
              ...
              <StackItem key={'request-requester'}>
                <TextContent>
                  <Text component={TextVariants.h6}>
                    {intl.formatMessage(requestsMessages.requesterColumn)}
                  </Text>
              ...
              <StackItem key={'request-order'}>
                <TextContent>
                  <Text component={TextVariants.h6}>
                    {intl.formatMessage(requestsMessages.orderNumber)}
                  </Text>
      ...
      <StackItem key={'request-parameters'}>
        <Card>
          <CardTitle>
            <Title headingLevel="h5" size="lg">
              {intl.formatMessage(requestsMessages.parameters)}
            </Title>
          </CardTitle>
          <CardBody>
            <Stack hasGutter>
              {requestContent.params &&
                Object.keys(requestContent.params).map((param) => {
                  return (
                    requestContent.params[param] && (
                      <StackItem
                        key={`request-${requestContent.params[param]}`}
                      >
                        <TextContent>
