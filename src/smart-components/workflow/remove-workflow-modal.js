...
const RemoveWorkflowModal = ({
  ...
  const removeWf = () =>
    (finalId
      ? dispatch(removeWorkflow(finalId, intl))
      : dispatch(removeWorkflows(ids, intl))
    )
  ...
  return (
    <Modal
      isOpen
      variant="small"
      aria-label={intl.formatMessage(worfklowMessages.removeProcessAriaLabel, {
      ...
      header={
        <Title size="2xl" headingLevel="h1">
          ...
          />
          {intl.formatMessage(worfklowMessages.removeProcessTitle, {
            count: finalId ? 1 : ids.length
          })}
        </Title>
      }
      ...
      <TextContent>
        <Text component={TextVariants.p}>
          {isLoading ? (
            ...
            intl.formatMessage(
              worfklowMessages.removeProcessDescriptionWithDeps,
              {
                name,
                dependenciesList: (
