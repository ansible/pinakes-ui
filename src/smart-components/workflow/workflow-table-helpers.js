...
import {
  repositionWorkflow as repositionWorkflowS,
  fetchWorkflows as fetchWorkflowsS,
  moveSequence as moveSequenceS
...
const debouncedMove = (cache, id) => {
  ...
  cache[id] = asyncDebounce(
    (workflow, dispatch, intl) =>
      dispatch(
        isStandalone()
          ? repositionWorkflowS(workflow, intl)
          : repositionWorkflow(workflow, intl)
      ).then(() =>
        dispatch(isStandalone() ? fetchWorkflowsS : fetchWorkflows())
      ),
    1500
  );
...
export const MoveButtons = ({ id }) => {
  ...
  const { isUpdating, property } = useSelector(
    ({ workflowReducer: { isUpdating, isLoading } }) => ({
      isUpdating: isUpdating > 0 || isLoading,
      property
    })
  );
  ...
  const updateSequence = (sequence) => {
    dispatch(
      isStandalone()
        ? moveSequenceS({ id, sequence })
        : moveSequence({ id, sequence })
    );
...
export const SelectBox = ({ id }) => {
  const { selectedWorkflows, setSelectedWorkflows } = useContext(
    WorkflowTableContext
  );
...
export const createRows = (data) =>
  data.map(({ id, name, description, sequence, group_refs }) => ({
    id,
    cells: [
